...
今回は Neovim のガワとして WindowsTerminal(WT) で説明しています。試していませんが
...
まず問題になるのが `1.`  
初回起動時なら WT に直接ファイルパスを含めたコマンドを送ればよいですが、すでに
Neovimが起動している場合、WT にコマンドを送ると新しい Neovim が開いてしまいます。
WT は起動しているが Neovim は未起動の場合もあるので、単純に WT の起動状態に
よってコマンドを分岐するのは良い方法ではありません。
<BR>
次に `2.`  
...
4. {string} NeovimのExコマンドを直接指定  
   `3.`がコマンド以外のときは引数としてパスを渡せる。パス区切りには`;`を使う  

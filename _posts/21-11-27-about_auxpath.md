...
この機能は大仰な言い方をすればPPcを外部コマンドの擬似的なフロントエンドにする機能で、
FTP、Androidデバイス、オンラインストレージ、バージョン管理システムなどで管理中
のファイルをPPcのエントリとして通常のディレクトリと大差なく操作感できます。

おもしろい機能ですが難解でもありヘルプの説明だけでは分からない点が多かった為、
TORO氏にHP掲示板で質問した内容や、自分で確認した範囲の解説をしていきます。
...
<!-- #### 注意点 -->
<!---->
<!-- aux:メニューに`overup`という項目があるが現状は多少わかりづらいものになっている。   -->
<!---->
<!-- 理由1. `overup`は`%si'RootPath'`より上位ディレクトリに移動したときに実行されるが、 -->
<!-- `aux:\S_○○○\`から上に移動したときには実行されず、`leave`が実行されたあとに、 -->
<!-- 元のパスに戻される。   -->
<!---->
<!-- 理由2. aux本体の付属ファイルppxaux.cfgを参照すると`base`に指定するパスの末尾には -->
<!-- パス区切りを付けることを想定されているように思えるが、実際に`base`の末尾に -->
<!-- パス区切りを付けると問題がある。   -->
<!-- たとえばbaseが`aux:S_base\`であるときに -->
<!---->

...
上から順に説明していきます。  
- 11行目 `NAME_LENGTH`は情報表示名の幅揃えです。表示名より短い場合は無視されます。  
<span style="background-color:gray;color:black;">Extension　　</span>:EXEX64,\*.EXE  
- 12行目`LINE_SEPARETOR`は見出しの区切り線です。一文字だけ`'`か`"`で括って指定します。  
不要であれば非表示にできます(後述)。
- 13行目`EX_COMMENT`は拡張コメントを表示する`true`か、しない`false`かです。`'"`で括りません。
- 14行目`EX_COMMENT_RAW`は拡張コメントを何行目から表示させるかです。  
通常は区切り線の下に表示されますが、区切り線を非表示にしている場合に参照されます。
- 61行目`info_tip`は見出し行です。ファイル名が指定されていますが、装飾文字などを加えることもできます。  
['<span style="background-color:gray;color:black;">ここに加えられます</span>' + PPx.Extract('%C')];
- 64行目からは情報の取得をします。上から順番に表示されます。  
